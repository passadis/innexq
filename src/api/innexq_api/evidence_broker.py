...
ROLES: tuple[Role, ...] = ("document_analyst", "equipment_service")
ALLOWED = {
...
class EvidenceBroker:
    ...
    def call(
        ...
        try:
            ...
            match = re.fullmatch(
                r"([0-9a-f]{32})\.(document_analyst|equipment_service)\.([A-Za-z0-9_-]{43})",
                raw,
            )
            if not match:
            ...
            if (
                ...
                and arguments["document_id"] not in binding.document_hashes
            ):
    ...
    def finish(
        ...
        operations: list[Any] = []
        if set(receipt_ids) != set(ROLES) or self.now() >= binding.expires_at:
        ...
        for role in ROLES:
            ...
            if role == "document_analyst" and {
                item["arguments"]["document_id"]
                for item in role_receipts
                if item["result"]["tool_name"] == "analyze_document"
