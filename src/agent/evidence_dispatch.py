...
from coverage_team import CoverageTeamGuard
...
class EvidenceDispatch(AgentMiddleware):
    def __init__(self, client: Any, credential: Any, endpoint: str, project: str) -> None:
        ...
        self.coverage = CoverageTeamGuard(client, credential, endpoint, project)
    ...
    async def process(
        ...
        if mode == "evidence_investigation":
            await self.candidate.process(context, call_next)
        elif mode == "coverage_renewal":
            await self.coverage.process(context, call_next)
        else:
